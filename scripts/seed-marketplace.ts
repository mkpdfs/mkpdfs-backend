...
export const templates: Omit<MarketplaceTemplate, 's3Key' | 'thumbnailKey' | 'createdAt' | 'updatedAt'>[] = [
  ...
  {
    templateId: 'mp-cert-corporate-diagonal',
    category: 'certificates',
    name: 'Corporate Diagonal Certificate',
    description: 'Professional corporate certificate with bold diagonal corner accents (navy + gold), recipient name in brand color, and a centered signature. Landscape A4, perfect for constancias and course completions.',
    tags: ['certificate', 'constancia', 'corporate', 'course'],
  ...
  {
    templateId: 'mp-cert-academic-qr',
    category: 'certificates',
    name: 'Academic Certificate (QR)',
    description: 'Clean university-style constancia with a soft grey swoosh background, large recipient name on a highlight band, QR verification code bottom-left, and one signature. Landscape A4.',
    tags: ['certificate', 'constancia', 'academic', 'qr', 'university'],
    orientation: 'landscape' as const,
    popularity: 0,
    sampleDataJson: JSON.stringify({
  ...
  },
  {
    ...
    orientation: 'landscape' as const,
    popularity: 0,
    sampleDataJson: JSON.stringify({ recipientName: 'Victor Manuel Coutiño Silva', courseName: 'Databricks Lakehouse Fundamentals' })
  },
  {
    templateId: 'mp-cert-ornate-frame',
    category: 'certificates',
    name: 'Ornate Frame Diploma',
    description: 'Classic Spanish-style diploma with an ornamental double border, decorative corner medallions in brand color, two-signature layout, and an ink stamp. Landscape A4.',
    tags: ['certificate', 'diploma', 'ornate', 'formal', 'frame'],
    orientation: 'landscape' as const,
    popularity: 0,
    sampleDataJson: JSON.stringify({ recipientName: 'Miguel Aramis Ramírez Ibarra', courseName: 'Máster en Programación' })
  },

  {
    ...
    sampleDataJson: JSON.stringify({
      ...
      courseName: 'Bootcamps de programación'
    })
  },
